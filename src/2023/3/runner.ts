function question1(input: string) {
  ...
  for (const [y, row] of input.split("\n").entries()) {
    const numbers = [...row.matchAll(/\d{1,}/g)].map((n) => ({
      number: n[0],
      x: n.index,
      y,
    }));
  ...
  const result = engineNumbersList.reduce((acc, { number, x, y }) => {
    ...
    xLoop: for (
      let x = searchIndicesRange.x.min;
      x <= searchIndicesRange.x.max;
      x++
    ) {
      for (
        let y = searchIndicesRange.y.min;
        y <= searchIndicesRange.y.max;
        y++
      ) {
...
function question2(input: string) {
  ...
  const result = gearsList.reduce((acc, { x, y }) => {
      ...
      for (
        ...
        if (numbersMap[x] && numbersMap[x][y]) {
          ...
          for (const existing of ajacencyList) {
            if (
              existing.originalX === foundNumber.originalX &&
              existing.y === foundNumber.y
            ) {
              isDuplicate = true;
              break;
            }
    ...
    if (ajacencyList.length === 2)
      return (
        acc +
        parseInt(ajacencyList[0].number) * parseInt(ajacencyList[1].number)
      );
    return acc;
  }, 0);
  // console.log(result);
